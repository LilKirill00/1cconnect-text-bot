          ...
          ticket_button:
              ...
              data: # Данные заявки
                ...
                description: # Описание
                  text: "{{ .User.Name }} Опишите что у вас случилось."
                  value: "Тут видите пример вставки шаблона: {{ .User.Name }}"
                executor: # Исполнитель
                  text: "Выберите специалиста который будет решать вашу проблему."
                service: # Услуга
                  text: "Текст приглашения к выбору услуги" 
                type: # Тип услуги
                  text: "Текст приглашения к вводу выбору типа работ"
```
...
- `text` - параметр который определяет какое сообщение будет на каждом шаге
- `value` - необязательный параметр. значение по умолчанию, которое будет использовано если нажать на кнопку перехода к следующему шагу

Примечание:
- параметры `value` для `executor | service | type` должны соответствовать тексту кнопок, которые будут сгенерированы на этих шагах. Например, если на этапе `executor` программа сгенерирует кнопку "Иванов Иван Иванович", то чтобы взять его в качестве исполнителя по умолчанию, нужно указать `value: "Иванов Иван Иванович"`.

Если в конфиге отсутствует `create_ticket_menu`, будет использовано меню по умолчанию. Данное меню позволит настроить кнопки которые видит пользователь на каждом шаге формирования заявки:
...
Обратите внимание:
- кнопка для перехода к следующему шагу и принятию значения по умолчанию из `value` обязательно должна содержать параметр `goto: create_ticket_menu`
- `answer` можно не настраивать тк он не работает во время формирования заявки
