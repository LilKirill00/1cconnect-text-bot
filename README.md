...
Можно настроить сообщение в случае возникновения ошибки в процессе работы кнопки через следующую строчку в конфиге бота (файл `bot.yml`), если данная настройка отсутствует, будет использовано сообщение по умолчанию:

```yaml
error_processing_message: 'Во время обработки вашего запроса произошла ошибка'
```
